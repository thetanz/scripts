...
echo "\"appname\",\"appid\",\"owner\",\"ownerObjectId\",\"upnState\",\"ownerList\"" > "${reportdir}/disabled-owners.csv"
...
echo "\"appname\",\"appid\",\"credExpiry\",\"credName\",\"ownerList\"" > "${reportdir}/expired-credentials.csv"
echo "\"appname\",\"appid\",\"consentPurpose\",\"consentType\",\"consentValue\",\"ownerList\"" > "${reportdir}/app-consents.csv"
echo "\"appname\",\"appid\",\"credName\",\"credExpiry\",\"ownerList\"" > "${reportdir}/excessive-duration-credentials.csv"
...
jq -c '.[]' "${jsondir}/apps.json" | while read app ; do
  ...
  echo "${app}" | jq -c > "${jsondir}/${appid}.json"
  ...
  appOwners=`az ad app owner list --id "${appid}" | jq -r '.[].mail // empty'`
  ownerList=`echo "${appOwners}" | tr '\n' ' '`
  ...
  # owners exist, so check em:
    ...
    echo "${appOwners}" | grep -v "^$" | while read owner ; do
      ...
      if [[ $owner == "a-"* ]] ; then
        warn "owner "${owner}" is domain admin of ${domain}"
        echo "\"${appname}\",\"${appid}\",\"${owner}\"" >> "${reportdir}/admin-theta-owners.csv"
      fi
